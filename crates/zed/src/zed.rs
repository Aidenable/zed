...
#[derive(Copy, Clone, Debug, settings::RegisterSetting)]
struct ReduceAnimationsSetting(bool);

impl Settings for ReduceAnimationsSetting {
    fn from_settings(content: &settings::SettingsContent) -> Self {
        Self(content.reduce_animations.unwrap_or_default())
    }
}

fn init_reduce_animations(cx: &mut App) {
    let apply = |cx: &mut App| cx.set_reduce_animations(ReduceAnimationsSetting::get_global(cx).0);
    apply(cx);
    cx.observe_global::<SettingsStore>(apply).detach();
}
